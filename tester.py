...
def decipher_as_receiver(ciphered_vector, modulo, multiplicative_to_mask,
                         private_key_vector):
    t = time.process_time()
    print(
        "\nAs a " + "RECEIVER" +
    ...
    deciphered_vector = deciphering.decipher_vector_elements(
        ciphered_vector, modulo, multiplicative_to_mask)
    ...
    for i in range(0, len(deciphered_vector)):
        ...
        deciphered_bit_sequence = deciphering.deciphered_items_to_bit_sequence(
            constants.algorithm_back_tracking, private_key_vector,
            deciphered_item)
    ...
    for i in range(0, len(deciphered_bit_sequences)):
        ...
        bit_to_text = utility.convert_bit_to_text(deciphered_bit_sequence,
                                                  len(private_key_vector))
        deciphered_text += bit_to_text

    print("Finished to decipher the text in " + str(time.process_time() - t) +
          " ms as a receiver.\n\n" + "Original text: " + str(deciphered_text))
    decrpted_text.insert(1.0, deciphered_text)
    return True
...
if __name__ == "__main__":
    ...
    encrypt_button = tk.Button(canvas,
                               text='Solve',
                               font=('verdana', 15),
                               command=lambda: encrypter(message_entry.get()))
